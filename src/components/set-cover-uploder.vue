        ...
        handlePictureCardPreview(file) {
            ...
            if (this.fileList.length) {
                this.fileList.forEach((item, index) => {
                    _this.newFilesUrl.push(item.url);
        ...
        handleChange(file, fileList) {
            ...
            reader.onload = function (e) {
                ...
                image.onload = function () {
                    ...
                    var ratio = _this.getPixelRatio(context); // 关键代码
                    canvas.width = image.width * ratio; // 画布宽度
                    canvas.height = image.height * ratio; // 画布高度
                    var dataUrl = "";
                    context.drawImage(image, 0, 0, canvas.width, canvas.height);
                    dataUrl = canvas.toDataURL("image/jpeg");
                    var files = _this.baseFile(dataUrl, file.name);
                    let form = new FormData();
                    form.append("file", files);
                    form.append("serviceId", 0);
                    form.append("serviceType", _this.types);
                    form.append("year", _this.years);
                    _this.$http.zdcUpload(form).then((res) => {
                        _this.loading = false;
                        if (res.data.code == 0) {
                            var obj = {
                                name: file.name,
                            ...
                            _this.$emit("changeId", res.data.data);
                        }else{
